...
function buildMagnetPrompt({ topic, audience, type, platform, byline }) {
...
WRITING RULES — no exceptions:
- No contractions: no I've, I'd, I'll, I'm, we've, they've, you've, it's, that's, don't, won't, can't, isn't
- No em dashes or double hyphens
- Short, punchy, actionable sentences. One idea per line.
- Real substance and specifics. No filler, no fluff.
...
app.post('/api/lead-magnet', async (req, res) => {
  const { uid, topic, audience, type, platform, byline, email } = req.body || {}
  ...
  const { system, user } = buildMagnetPrompt({ topic, audience, type, platform, byline })
